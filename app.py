...
if uploaded_file is not None:
    # Button to force a manual refresh if needed
    if not st.session_state.gem_registry or st.sidebar.button("🔄 Clear & Re-Scan Document"):
        st.session_state.gem_registry = {}
        
        with st.spinner("Analyzing layout grid and capturing full gem boxes..."):
            doc = fitz.open(stream=uploaded_file.read(), filetype="pdf")
            
            for page_num in range(len(doc)):
                page = doc[page_num]
                
                # Check for multiple case variations of the serial prefix
                text_instances = page.search_for("S.No")
                if not text_instances:
                    text_instances = page.search_for("S.NO")

                for inst in text_instances:
                    # Expand horizontal search window slightly to guarantee grabbing the full number digits
                    text_window = page.get_text("text", clip=fitz.Rect(inst.x0, inst.y0 - 5, inst.x1 + 180, inst.y1 + 15))
                    ...
                    # --- EXPANDED GRID CROPPING MATH ---
                    # Re-engineered margins to grab the entire upper block completely (prevent cutting half images)
                    crop_rect = fitz.Rect(
                        inst.x0 - 40,   # Pushed further left to avoid slicing the side of the stone
                        inst.y0 - 340,  # Lifted drastically higher (340px) to catch the top of taller gems
                        inst.x1 + 320,  # Extended right to ensure full image width is encapsulated
                        inst.y1 + 160   # Brought lower to wrap text description tags safely
                    )
                    
                    # Prevent going past actual physical page limits
            ...
            if st.button("Generate & Download My Custom PDF"):
                with st.spinner("Compiling pages..."):
                    ...
                    for s_no in selected_gems:
                        ...
                        display_window = fitz.Rect(40, 80, 555, 750)
